...
function Sf:Drive(model, destination, value, t)
	...
	for i = 1, #waypoints do
		...
		-- คำนวณทิศทางหันหน้าจาก waypoint ปัจจุบัน → ถัดไป
		local facingCF
		if i < #waypoints then
			local nextPos = waypoints[i + 1].Position
			local flat = Vector3.new(nextPos.X - wp.Position.X, 0, nextPos.Z - wp.Position.Z)
			if flat.Magnitude > 0.01 then
				facingCF = CFrame.lookAt(Vector3.zero, flat)
			end
		end
		...
		while true do
			...
			local newPos = curPos + moveDir * step

			-- หันหน้าตาม waypoint ถัดไป ถ้าไม่มีให้ใช้ moveDir
			local facing = facingCF and facingCF.LookVector or moveDir
			local flatFacing = Vector3.new(facing.X, 0, facing.Z)
			if flatFacing.Magnitude < 0.01 then
				flatFacing = Vector3.new(moveDir.X, 0, moveDir.Z)
			end

			local newCF = CFrame.new(newPos) * CFrame.fromMatrix(
				Vector3.zero,
				flatFacing:Cross(Vector3.yAxis).Unit * -1,
				Vector3.yAxis,
				flatFacing.Unit * -1
			)
			model:PivotTo(newCF)
						...
						if closestATM and c().AutoFarmATM then
							...
							print("[DEBUG] dist to ATM =", dist)
								...
								print("[DEBUG] calling exit_seat")
									...
									-- เช็คระยะก่อนลงรถ ถ้ายังไกลเกิน 12 ให้ Drive อีกรอบ
									local distNow = (closestATM.Area.Position - RootPart.Position).Magnitude
									print("[DEBUG] dist before exit =", distNow)
									if distNow > 12 then
										Sf:Drive(vehicle, closestATM.Area.Position, c().AutoFarmATM, closestATM)
										if not c().AutoFarmATM then break end
										task.wait(0)
