-- ========================================
-- RARITY MONITOR + DISCORD WEBHOOK (รวมตัวเอง + Debug ครบ)
-- ========================================
print("[DEBUG] Loading Rarity Monitor...")

local PlayersService = game:GetService("Players")
local HttpService = game:GetService("HttpService")
local RunService = game:GetService("RunService")

local WEBHOOK_URL = "https://discord.com/api/webhooks/1508469549450789056/pR3yNS2PvwlhPeB99awdnV6WZIg1GtZq38QKyPr_xsH77m7XSwcfujsZAiK2mruPownV"

local RARITY_THRESHOLD = {
    Epic = true,
    Legendary = true,
    Mythic = true
}
...
if not _G.WeaponRegistry then
    _G.WeaponRegistry = {}
    print("[DEBUG] Created global WeaponRegistry")
end

local WeaponRegistry = _G.WeaponRegistry
...
local function getItemKey(tool)
    ...
    local itemName = tool:GetAttribute("DisplayName") or tool:GetAttribute("WeaponName") or tool.Name
    return "NAME_" .. itemName .. "_RARITY_" .. rarity
end
...
local function getWeaponInfo(tool)
    ...
    if not info then
        info = {
            Name = tool:GetAttribute("DisplayName") or tool:GetAttribute("WeaponName") or tool.Name,
            Rarity = tool:GetAttribute("RarityName") or "Common",
            ImageId = tool:GetAttribute("ImageId") or "",
            ToolName = tool.Name
        }
        WeaponRegistry[key] = info
        print("[DEBUG] Created weapon info for", info.Name, "Rarity:", info.Rarity)
    end

    return info
end

local function sendRarityAlert(playerName, itemName, rarity, jobId, isSelf)
    local prefix = isSelf and "⭐ SELF - " or "🔥 "
    
    print(string.format("[DEBUG] >>> SENDING ALERT: %s%s has %s (%s) in server %s", 
        prefix, playerName, itemName, rarity, jobId))
    
    local color = rarity == "Epic" and 0x9b59b6 or 
                  rarity == "Legendary" and 0xf1c40f or 
                  rarity == "Mythic" and 0xe74c3c or 0x3498db

    local title = (isSelf and "⭐ YOUR RARE ITEM DETECTED! ⭐") or "🔥 Rare Item Detected!"
    ...
    local embed = {
        ...
        fields = {
            ...
            {name = "🔫 Item", value = tostring(itemName), inline = false},
            {name = "✨ Rarity", value = tostring(rarity), inline = true},
            {name = "🆔 Job ID", value = "`" .. tostring(jobId) .. "`", inline = false}
        },
        footer = {text = "Rarity Monitor | " .. os.date("%Y-%m-%d %H:%M:%S")}
    }

    local success, err = pcall(function()
        local data = HttpService:JSONEncode({username = "Rarity Monitor", embeds = {embed}})
        local headers = {["Content-Type"] = "application/json"}
        local req = (syn and syn.request) or http_request or request
        
        if req then
            req({Url = WEBHOOK_URL, Method = "POST", Headers = headers, Body = data})
        else
    ...
    if not success then
        warn("[DEBUG] Webhook failed:", err)
    else
        print("[DEBUG] Webhook sent successfully!")
    end
end

local function scanPlayersForRareItems()
    print("\n[DEBUG] ========== SCANNING FOR RARE ITEMS ==========")
    local now = os.time()
    local currentJobId = game.JobId
    local players = PlayersService:GetPlayers()
    
    print(string.format("[DEBUG] Total players in server: %d (including you)", #players))
    
    for _, player in ipairs(players) do
        local isSelf = (player == PlayersService.LocalPlayer)
        local selfTag = isSelf and "[YOU] " or ""
        
        local tools = getPlayerTools(player)
        print(string.format("[DEBUG] %s%s has %d tools", selfTag, player.Name, #tools))
        
        if #tools == 0 then
            print(string.format("[DEBUG] %s%s - No tools found", selfTag, player.Name))
        end
        
        for _, tool in ipairs(tools) do
            local info = getWeaponInfo(tool)
            if info and info.Rarity then
                print(string.format("[DEBUG] %s%s - Tool: %s, Rarity: %s", 
                    selfTag, player.Name, info.Name, info.Rarity))
                
                if RARITY_THRESHOLD[info.Rarity] then
                    local key = player.UserId .. "_" .. info.Name
                    local lastTime = lastAlertTime[key] or 0
                    
                    if now - lastTime >= COOLDOWN_SECONDS then
                        print(string.format("[DEBUG] ⚠️ RARE ITEM FOUND! %s%s - %s (%s) - Sending alert...", 
                            selfTag, player.Name, info.Name, info.Rarity))
                        lastAlertTime[key] = now
                        sendRarityAlert(player.Name, info.Name, info.Rarity, currentJobId, isSelf)
                    else
                        local remaining = COOLDOWN_SECONDS - (now - lastTime)
                        print(string.format("[DEBUG] ⏰ Skipped (cooldown %ds left): %s%s - %s", 
                            remaining, selfTag, player.Name, info.Name))
                    end
                else
                    print(string.format("[DEBUG] %s%s - %s (Rarity: %s - below threshold)", 
                        selfTag, player.Name, info.Name, info.Rarity))
                end
            else
                print(string.format("[DEBUG] %s%s - Tool has no rarity info", selfTag, player.Name))
            end
        end
    end
...
-- เมื่อผู้เล่นออกจากเซิร์ฟเวอร์ ให้ล้าง cooldown
PlayersService.PlayerRemoving:Connect(function(player)
    print("[DEBUG] Player leaving:", player.Name)
    for k in pairs(lastAlertTime) do
        if k:find(player.UserId .. "_") then
            lastAlertTime[k] = nil
            print("[DEBUG] Cleared cooldown for", player.Name)
        end
    end
end)

-- เริ่ม loop สแกนทุก 10 วินาที
task.spawn(function()
    print("[DEBUG] Rarity Monitor started. Scanning every 10 seconds.")
    while true do
        task.wait(10)
        local success, err = pcall(scanPlayersForRareItems)
        if not success then
            warn("[DEBUG] Error in scanPlayersForRareItems:", err)
        end
    end
end)

-- สแกนครั้งแรกหลังจาก 2 วินาที
task.wait(2)
pcall(scanPlayersForRareItems)

print("[DEBUG] Rarity Monitor fully loaded! Will scan every 10 seconds.")
print("[DEBUG] Threshold rarities: Epic, Legendary, Mythic")
print("[DEBUG] Cooldown: " .. COOLDOWN_SECONDS .. " seconds per player/item")-- โค้ดเต็มหลังจากแก้ไขฟังก์ชัน Sf:Drive ให้หันหน้าไปตามทิศทางของ waypoints (แบบ tween ตรง ๆ)
...
function Sf:Drive(model, destination, value, t)
    ...
    for i, wp in pairs(waypoints) do
        ...
        while shouldContinue(value) do
            ...
            -- ✅ อ่าน position จริงจาก model ทุก frame แล้วค่อย update distToGoal
            local currentPos = model:GetPivot().Position
            local moveDelta = goalPos - currentPos
            local distToGoal = moveDelta.Magnitude

            if distToGoal <= 0.5 then break end  -- ✅ break หลัง update distToGoal จริงๆ
            ...
            local step = speed * dt
            local newPos = currentPos + moveDirection * math.min(step, distToGoal)

            -- ✅ ตั้ง CFrame ให้หันหน้าไปตามทิศทางที่กำลังเคลื่อนที่
