...
do
    ...
    local AA_HISTORY_SIZE  = 8
    local AA_Y_THRESHOLD   = 200
    local AA_COUNT_NEEDED  = 5
    local AA_TICK_INTERVAL = 0.05
    local AA_lastTick      = 0
    ...
    RunService.Heartbeat:Connect(function()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if everDetected[player] then isAntiAim[player] = true end
            local vel = root.AssemblyLinearVelocity
            velHistory[player] = velHistory[player] or {}
            table.insert(velHistory[player], { time = now, vel = vel })
            if #velHistory[player] > AA_HISTORY_SIZE then table.remove(velHistory[player], 1) end
            local yHigh = math.abs(vel.Y) > AA_Y_THRESHOLD
            ...
            if (antiAimCount[player] or 0) >= AA_COUNT_NEEDED then
