...
_bringCorePart.Name = "BringCore"; _bringCorePart.Anchored = true
_bringCorePart.CanCollide = false; _bringCorePart.Transparency = 1
...
Torque.Torque = Vector3.new(100000, 100000, 100000); Torque.Attachment0 = Att2
AlignPos.MaxForce = math.huge; AlignPos.MaxVelocity = math.huge
AlignPos.Responsiveness = 9999; AlignPos.Attachment0 = Att2; AlignPos.Attachment1 = _bringAttachment1
end
end
...
local BringConnRef = nil

local function toggleBringPlayer(playerName, state)
BringActive = state
if BringConnRef then BringConnRef:Disconnect(); BringConnRef = nil end
if not state then return end
local target = getPlayerByName(playerName)
if not target then print("ไม่พบผู้เล่น: "..playerName) BringActive = false return end
...
BringConnRef = workspace.DescendantAdded:Connect(function(v) pcall(_forcePart, v) end)
task.spawn(function()
...
local BringToggle = CombatTab:Toggle({
    Title = "Bring Player",
    Default = false,
    Callback = function(state)
        local playerName = BringPlayerBox.Value or ""
        toggleBringPlayer(playerName, state)
    end
})

Config:Register("BringPlayer", BringToggle)

local BringPlayerBox = CombatTab:Input({
    Title = "Player Name",
    Placeholder = "ใส่ชื่อผู้เล่น",
    Callback = function(text)
        selectedBringPlayer = text
    end
})
...
local BringToggle = CombatTab:Toggle({
    ...
    Default = false,
    Callback = function(state)
        toggleBringPlayer(selectedBringPlayer, state)
    end
})
