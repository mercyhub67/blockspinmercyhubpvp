...
local isAntiAim      = {}
local antiAimCount   = {}
local lastDetectTime = {}
local everDetected   = {}

local HISTORY_SIZE  = 8
local Y_THRESHOLD   = 200
local COUNT_NEEDED  = 5
local TICK_INTERVAL = 0.05
local lastTick      = 0

local function getOrCreateLabel(player)
    if antiAimLabels[player] then return antiAimLabels[player] end
    local label = Drawing.new("Text")
    label.Size    = 14
    label.Center  = true
    label.Outline = true
    label.Color   = Color3.fromRGB(180, 0, 255)
    label.Font    = 4
    label.Visible = false
    label.Text    = "⚠ Hacker"
    antiAimLabels[player] = label
    return label
end
...
RunService.Heartbeat:Connect(function()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if everDetected[player] then
            isAntiAim[player] = true
        end

        local vel = root.AssemblyLinearVelocity
        velHistory[player] = velHistory[player] or {}
        table.insert(velHistory[player], { time = now, vel = vel })
        if #velHistory[player] > HISTORY_SIZE then
            table.remove(velHistory[player], 1)
        end

        local yHigh = math.abs(vel.Y) > Y_THRESHOLD
        local xzMag = Vector2.new(vel.X, vel.Z).Magnitude

        local xzSpinning = false
        if #velHistory[player] >= 3 then
            local h  = velHistory[player]
            local v1 = Vector2.new(h[#h-2].vel.X, h[#h-2].vel.Z)
            local v2 = Vector2.new(h[#h-1].vel.X, h[#h-1].vel.Z)
            local v3 = Vector2.new(h[#h].vel.X,   h[#h].vel.Z)
            if v1.Magnitude > 30 and v2.Magnitude > 30 and v3.Magnitude > 30 then
                local dot = v1:Dot(v3) / (v1.Magnitude * v3.Magnitude + 1e-6)
                xzSpinning = dot < 0.7
            end
        end

        local yFlipping = false
        if #velHistory[player] >= 3 then
            local h  = velHistory[player]
            local y1 = h[#h-2].vel.Y
            local y3 = h[#h].vel.Y
            yFlipping = (y1 > 70 and y3 < -70) or (y1 < -70 and y3 > 70)
        end

        if yHigh or xzSpinning or yFlipping then
            antiAimCount[player] = (antiAimCount[player] or 0) + 1
            lastDetectTime[player] = now
            print(("[ANTI-AIM] %s | Y=%.1f XZ=%.1f spin=%s flip=%s count=%d"):format(
                player.Name, vel.Y, xzMag, tostring(xzSpinning), tostring(yFlipping), antiAimCount[player]
            ))
        end

        if (antiAimCount[player] or 0) >= COUNT_NEEDED then
            if not isAntiAim[player] then
                print(("[ANTI-AIM DETECTED] %s"):format(player.Name))
            end
            isAntiAim[player]    = true
            everDetected[player] = true
        end
...
Players.PlayerRemoving:Connect(function(player)
    ...
    isAntiAim[player]      = nil
    lastDetectTime[player] = nil
...
local function resetPlayerData(player)
    ...
    lastDetectTime[player] = nil
    if antiAimLabels[player] then
        pcall(function() antiAimLabels[player].Visible = false end)
    end
    print(("[RESET] %s respawned — data cleared"):format(player.Name))
end
...
local function watchPlayer(player)
    ...
    player.CharacterAdded:Connect(function()
        ...
        task.wait(2)
        print(("[WATCH] เริ่มเช็ค %s หลังรีตัว"):format(player.Name))
    end)
end
