...
-- ========================================
-- RARITY MONITOR + DISCORD WEBHOOK
-- ========================================

local PlayersService = game:GetService("Players")
local HttpService    = game:GetService("HttpService")

local WEBHOOK_URL       = "https://discord.com/api/webhooks/1508477067585650791/5bo-CfwaUpRL3GWwFVEl1HNOknvmMd5KEei_vNdupxUrVH8t0houHPtdfGvKmiqi9Yej"
local RARITY_THRESHOLD  = { Epic = true, Legendary = true, Mythic = true }
local lastAlertTime     = {}
local COOLDOWN_SECONDS  = 60

if not _G.WeaponRegistry then _G.WeaponRegistry = {} end
...
-- Fix 1: ไม่ใช้ os.date (บัคภาษาไทย) ใช้ tick() แทน
...
-- Fix 2: เปลี่ยน JobId เป็น ServerName
local function getServerName()
    local ok, name = pcall(function()
        return game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name
    end)
    return ok and name or "Unknown Server"
end

local serverName = getServerName()

local function sendRarityAlert(playerName, itemName, rarity, jobId)
    local colorMap = {
        Epic      = 0x9b59b6,
        Legendary = 0xf1c40f,
        Mythic    = 0xe74c3c,
    ...
    local embed = {
        ...
        color  = colorMap[rarity] or 0x3498db,
        fields = {
        ...
        footer = { text = "Rarity Monitor - " .. getTimeString() }
    }
    ...
    pcall(function()
        ...
        if syn and syn.request then
            syn.request({ Url = WEBHOOK_URL, Method = "POST", Headers = headers, Body = data })
        elseif request then
            request({ Url = WEBHOOK_URL, Method = "POST", Headers = headers, Body = data })
        else
...
local function scanPlayersForRareItems()
    ...
    for _, player in ipairs(PlayersService:GetPlayers()) do
        ...
        for _, tool in ipairs(getPlayerTools(player)) do
            local info = getWeaponInfo(tool)
            if info and RARITY_THRESHOLD[info.Rarity] then
                local key      = player.UserId .. "_" .. info.Name
                local lastTime = lastAlertTime[key] or 0
                if now - lastTime >= COOLDOWN_SECONDS then
                    lastAlertTime[key] = now
                    sendRarityAlert(player.Name, info.Name, info.Rarity, currentJobId)
                end
            end
        end
    end
...
PlayersService.PlayerRemoving:Connect(function(player)
    for k in pairs(lastAlertTime) do
        if k:find(tostring(player.UserId) .. "_") then
            lastAlertTime[k] = nil
        end
    end
end)
