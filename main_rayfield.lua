...
do
    ...
    normalBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            if splashGui then
                ...
                pressButton(frame and frame:FindFirstChild("PlayButton"))
    ...
    godBtn.MouseButton1Click:Connect(function()
        destroyUI()
        task.spawn(function()
            require(_ReplicatedStorage.Modules.Game.CharacterCreator.CharacterCreator)
            require(_ReplicatedStorage.Modules.Core.Char)
            ...
            if characterCreator then
                ...
                pressButton(menuFrame and menuFrame:FindFirstChild("AvatarMenuSkipButton"))
...
-- ── Global env helper ────────────────────────────────────────
local getEnv = (type(getgenv) == "function" and getgenv) or function() return _G
...
local WindUI = nil -- compat stub

local Window = Rayfield:CreateWindow({
    Name             = "MERCY HUB  |  Block Spin",
    Icon             = 0,
    LoadingTitle     = "MERCY HUB",
    LoadingSubtitle  = "Block spin | Paid",
    Theme            = "DarkBlue",
...
local Config = {
    ...
    Save     = function() Rayfield:SaveConfiguration() end,
    Load     = function() Rayfield:LoadConfiguration() end,
    Delete   = function() end,
}
...
local function callRemote(remote, ...)
    ...
    elseif remote.ClassName == "RemoteFunction" then
        ...
        safeCall(function(...)
            ...
            if remote.InvokeServer then remote:InvokeServer(...)
...
local function netGet(...)
    if not Counter or not Counter.func then return
        ...
        if typeof(v) == "Instance" then
            if v:IsA("Model") and #v:GetChildren() == 0 then
                ...
                if dropped then
                    ...
                    if model then args[i] = model else return
    ...
    if not ok then warn("[NetGet Error]", result)
...
local function calculateVelocity(player)
    ...
    if not history or #history < 2 then return Vector3.new()
    ...
    if totalWeight == 0 then return Vector3.new()
...
local function predictPosition(part, root)
    if not part then return Vector3.zero
...
local function isBehindWall(origin, target)
    if not origin or not target then return false

    local dir = target - origin
    if dir.Magnitude < 1 then return false
                    ...
                    local function isShotgun()
                        ...
                        for _, tool in ipairs(Character:GetChildren()) do
                            if tool:IsA("Tool") then
                                ...
                                if ammo == "shotgun" or ammo == "shootgun" then return true
...
local function setupCharacter(char)
    ...
    if jumpConn then pcall(function() jumpConn:Disconnect() end)
...
local function isDowned()
    ...
    if not hum or hum.Health <= 0 then return false
...
local function flickerAndMove()
    if isFlickering then return

    isFlickering = true
    task.spawn(function()
        while isFlickering and antiKillEnabled and isDowned() do
            local hum = CharModule.get_hum()
            if hum and hum.Health <= 0 then break
...
RunService.Heartbeat:Connect(function()
    if not antiKillEnabled then return
    ...
    if isDowned() then
        ...
        if underMapPos then
            local root = getHRP()
            if root then root.CFrame = underMapPos + Vector3.new(0, 40, 0)
...
local function checkAndPickup()
    if not autoPickupEnabled then return
...
local function getPlayersInRange(range)
    local result = {}
    local myChar = LocalPlayer.Character
    if not myChar or not myChar.PrimaryPart then return result
...
local function isMeleeToolCheck(tool)
    if not tool then return false

    if tool.Name == "Fists" then return true
...
local function attackNearby()
    if not SendRemote then return

    local myChar = LocalPlayer.Character
    if not myChar or not myChar.PrimaryPart then return

    local tool = getActiveTool()
    if not tool or not isMeleeToolCheck(tool) then return

    local ok, parent = pcall(function() return tool.Parent end)
    if not ok or parent ~= LocalPlayer.Character then return

    local nearby = getPlayersInRange(20)
    if #nearby == 0 then return

    local ok2, myPos = pcall(function() return myChar.PrimaryPart.Position end)
    if not ok2 or not myPos then return
...
local function startAutoAttack()
    if autoAttackRunning then return
...
local function toggleSnap()
    if not snapUnderMapEnabled then return
...
local function lockYPosition()
    if snapConn then pcall(function() snapConn:Disconnect() end)
...
local function tryHoldPrompt(prompt, duration)
    if not prompt or prompt:GetAttribute("__AutoFinishBusy") then return
...
local function applyToAll()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if root then
                local prompt = root:FindFirstChild("FinishPrompt")
                if prompt then setFinishPrompt(prompt)
...
for _, player in ipairs(Players:GetPlayers()) do setupFastFinishForPlayer(player)
...
for _, p in pairs(Players:GetPlayers()) do createESP(p)

Players.PlayerAdded:Connect(createESP)
Players.PlayerRemoving:Connect(function(player)
    local data = ESP[player]
    if data then
        if data.Highlight then data.Highlight:Destroy()

        if data.Name      then data.Name:Remove()     

        if data.Info      then data.Info:Remove()     

        if data.HpBar     then data.HpBar:Remove()    

        if data.HpBg      then data.HpBg:Remove()     
...
local function getItemKey(tool)
    ...
    if handle then
        local mesh = handle:FindFirstChildOfClass("SpecialMesh")
        if mesh and mesh.MeshId ~= "" then return mesh.MeshId .. (mesh.TextureId or "") .. "_RARITY_" .. rarity

        if handle:IsA("MeshPart") and handle.MeshId ~= "" then return handle.MeshId .. (handle.TextureID or "") .. "_RARITY_" .. rarity

   

    if itemId and itemId ~= "" and itemId ~= tool.Name then return "ITEMID_" .. itemId .. "_RARITY_" .. rarity

    return "NAME_" .. displayName .. "_" .. tool.Name .. "_RARITY_" .. rarity
end

local function getWeaponInfo(tool)
    if not tool or not tool:IsA("Tool") then return nil
    ...
    for _, bagName in ipairs({"Backpack", "StarterGear", "StarterPack"}) do
        ...
        if bag then
            for _, t in ipairs(bag:GetChildren()) do
                if t:IsA("Tool") and t.Name ~= "Fists" then table.insert(tools, t)
...
RunService.RenderStepped:Connect(function()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player == Players.LocalPlayer then continue

        local char = player.Character
        if not char then clearInventoryDrawings(player) continue

        local head = char:FindFirstChild("Head")
        local root = char:FindFirstChild("HumanoidRootPart")
        if not head or not root then clearInventoryDrawings(player) continue
        ...
        for _, tool in ipairs(tools) do
            ...
            if info then table.insert(infos, info)
...
for _, folderName in ipairs({"gun", "melee", "throwable", "consumable", "farming", "misc", "rod", "fish"}) do
    ...
    if folder then registerItems(folder)
...
local function isGunTool(tool)
    if not tool or not tool:IsA("Tool") then return false
...
local function FindFireRateAttribute(gun)
    if not gun then return "fire_rate"

    if gun:GetAttribute("fire_rate") ~= nil then return "fire_rate"
...
local function applyGodGun(tool)
    if not tool or not isGunTool(tool) then return
    ...
    task.spawn(function()
        for _ = 1, 20 do
            ...
            local keys = {}
            for k in pairs(attrs) do table.insert(keys, k)

            table.sort(keys)
            if #keys >= 11 then
                local targetKey = keys[11]
                for _ = 1, 5 do forceSetAttr(tool, targetKey, true); task.wait(0.01)
...
RunService.Heartbeat:Connect(function()
    if not getEnv().GunModsAutoApply then return

    local char = LocalPlayer.Character
    if not char then return

    for _, tool in ipairs(char:GetChildren()) do
        if tool:IsA("Tool") and isGunTool(tool) then pcall(applyGodGun, tool)
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    repeat
        task.wait(0.1)
        for _, tool in ipairs(char:GetChildren()) do
            if tool:IsA("Tool") and isGunTool(tool) then task.spawn(applyGodGun, tool)
...
-- ══════════════════════════════════════════════════════════════
local meleeNames = {}
for _, tool in ipairs(MeleeItems:GetChildren()) do table.insert(meleeNames, tool.Name)


local function isMeleeByName(tool)
    if not tool:IsA("Tool") then return false

    if tool.Name == "Fists" then return true

    for _, name in ipairs(meleeNames) do
        if tool.Name == name then return true
...
local function modifyFists(tool, enable)
    ...
    if #keys >= 7 then
        ...
        local dmgKey   = keys[7]
        if enable then
            if originalAttribs[rangeKey] == nil then originalAttribs[rangeKey] = tool:GetAttribute(rangeKey)

            if originalAttribs[dmgKey]   == nil then originalAttribs[dmgKey]   = tool:GetAttribute(dmgKey)  

            tool:SetAttribute(rangeKey, 360)
            tool:SetAttribute(dmgKey,   20)
        else
            if originalAttribs[rangeKey] then tool:SetAttribute(rangeKey, originalAttribs[rangeKey])

            if originalAttribs[dmgKey]   then tool:SetAttribute(dmgKey,   originalAttribs[dmgKey])  
...
local function checkAndModifyFists()
    local char    = LocalPlayer.Character
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    if not char or not backpack then return

    for _, tool in ipairs(char:GetChildren()) do
        if isMeleeByName(tool) then modifyFists(tool, meleeAuraEnabled)
...
RunService.Heartbeat:Connect(function()
    if meleeAuraEnabled then checkAndModifyFists()
...
local function setupHighJump(char)
    if not jumpPowerEnabled then return

    local humanoid = char:WaitForChild("Humanoid")
    local hrp = char:WaitForChild("HumanoidRootPart")
    humanoid.UseJumpPower = true
    humanoid.JumpPower = 60
    if jumpConn_HJ then pcall(function() jumpConn_HJ:Disconnect() end)
...
LocalPlayer.CharacterAdded:Connect(function(char)
    if jumpPowerEnabled then setupHighJump(char)
...
local function setupBackpack(backpack)
    if not backpack then return

    for _, tool in ipairs(backpack:GetChildren()) do lockTool(tool)
...
local function initBackpack()
    ...
    if backpack then
        ...
        LocalPlayer.ChildAdded:Connect(function(child)
            if child:IsA("Backpack") then setupBackpack(child)
...
if ok and sellBtn then
    ...
    if holdStroke then
        ...
        if grad then grad.Enabled = false

   

    for _, v in pairs(sellBtn:GetDescendants()) do
        if v:IsA("NumberValue") then v.Value = 1
...
local function hookButton(btn)
    if not btn then return

    if btn:FindFirstChild("UnlocksAtText") then btn.UnlocksAtText.Visible = false

    if btn:FindFirstChild("EmoteName")     then btn.EmoteName.Visible = true     
...
if LocalPlayer.Character then setupCharacter(LocalPlayer.Character)
...
CombatTab:CreateSection("GUN:")

local SilentAimToggle = CombatTab:CreateToggle({
    Title   = "Silent Aim",
    CurrentCurrentOption = false,
    Callback = function(v)
        silentAimEnabled = v
        aimTarget = nil
    end,
...
local RedLineToggle = CombatTab:CreateToggle({
    Title   = "Red Line Lock",
    CurrentCurrentOption = false,
    Callback = function(v)
...
local FOVSlider = CombatTab:CreateSlider({
    Title = "FOV: ",
    Step  = 1,
    CurrentOption = { Min = 20, Max = 800, CurrentValue = fovRadius },
    Callback = function(v)
...
pcall(function() CombatTab:CreateDivider() end)

-- ── TAB: WEAPON ───────────────────────────────────────────────
local WeaponTab = Window:CreateTab("WEAPON:")
WeaponTab:CreateSection("MODS:")

WeaponTab:CreateSlider({
    Title = "Fire Rate",
    Step  = 10,
    CurrentOption = { Min = 100, Max = 3000, CurrentValue = 1000 },
    Callback = function(v) getEnv().FireRateCurrentOption = v end,
})

WeaponTab:CreateSlider({
    Title = "Accuracy",
    Step  = 0.01,
    CurrentOption = { Min = 0, Max = 1, CurrentValue = 1 },
    Callback = function(v) getEnv().AccuracyCurrentOption = v end,
})

WeaponTab:CreateSlider({
    Title = "Recoil",
    Step  = 0.1,
    CurrentOption = { Min = 0, Max = 10, CurrentValue = 0 },
    Callback = function(v) getEnv().RecoilCurrentOption = v end,
})

WeaponTab:CreateSlider({
    Title = "Reload Time",
    Step  = 0.1,
    CurrentOption = { Min = 0.1, Max = 10, CurrentValue = 0.1 },
    Callback = function(v) getEnv().ReloadCurrentOption = v end,
})
...
WeaponTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        Rayfield:Notify({ Title = "✅ Auto Modify", Content = "", Duration = 2 })

    end,
})

WeaponTab:CreateSection("COMBAT")

Config:Register("Fists Modifier", WeaponTab:CreateToggle({
    Title   = "Melee Aura",
    Desc    = "WideFists",
    CurrentCurrentOption = false,
    Callback = function(v)
...
local AutoAttackToggle = WeaponTab:CreateToggle({
    Title   = "Auto Attack",
    CurrentCurrentOption = false,
...
local InventoryESPToggle = ESPTab:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            ...
            Rayfield:Notify({ Title = "✅ ESP Items Enabled", Content = "", Duration = 3 })

        else
            if inventoryConn then inventoryConn:Disconnect(); inventoryConn = nil

            for _, gui in pairs(PlayerBillboards) do gui:Destroy()

            PlayerBillboards = {}
            Rayfield:Notify({ Title = "❌ ESP Items Disabled", Content = "", Duration = 3 })
...
local HighlightESPToggle = ESPTab:CreateToggle({
    Title   = "Highlight",
    CurrentCurrentOption = false,
    Callback = function(v)
        highlightEnabled = v
...
local DesyncBtn = CharTab:CreateButton({
...
local WalkSpeedToggle = CharTab:CreateToggle({
    Title   = "Walk Speed",
    CurrentCurrentOption = false,
...
local SpeedSlider = CharTab:CreateSlider({
    ...
    Step  = 0.5,
    CurrentOption = { Min = 1, Max = 5, CurrentValue = 2 },
    Callback = function(v) speedMultiplier = v * 0.05 end,
})
Config:Register("SpeedMultiplier", SpeedSlider)

local JumpToggle = CharTab:CreateToggle({
    Title   = "High Jump",
    CurrentCurrentOption = false,
    Callback = function(v)
        jumpPowerEnabled = v
        if v then
            if LocalPlayer.Character then setupHighJump(LocalPlayer.Character)

        else
            if jumpConn_HJ then pcall(function() jumpConn_HJ:Disconnect() end); jumpConn_HJ = nil
...
local AutoSprintToggle = CharTab:CreateToggle({
        ...
        if v then
            ...
            if ok and SprintModule then
                ...
                if sprintBar then
                    ...
                    sprintBar.update = function(...) return origUpdate(function() return 1 end)
                    ...
                    Rayfield:Notify({ Title = "✅ INF STAMINA", Content = "", Duration = 3 })
            ...
            Rayfield:Notify({ Title = "❌ Auto Sprint Disabled", Content = "", Duration = 3 })
...
local AntiLockToggle = CharTab:CreateToggle({
    Title   = "Anti Lock",
    CurrentCurrentOption = false,
    Callback = function(v)
        antiLockEnabled = v
        getEnv().Sky = v
        if v then getEnv().SkyAmount = 1500

    end,
})
Config:Register("AntiLock", AntiLockToggle)

local AntiKillToggle = CharTab:CreateToggle({
    Title   = "Anti Kill",
    CurrentCurrentOption = false,
    Callback = function(v)
        antiKillEnabled = v
        do
            Rayfield:Notify({ Title = v and " Anti Kill Enabled" or "❌ Anti Kill Disabled", Duration = 3 })
...
pcall(function() if CharTab and typeof(CharTab.Divider) == "function" then CharTab:CreateDivider() end end)
pcall(function() if CharTab and typeof(CharTab.Section) == "function" then CharTab:CreateSection("Att:") end end)

local PickupToggle = CharTab:CreateToggle({
    Title   = "Pickup items",
    CurrentCurrentOption = false,
...
local AntiRagdollToggle = CharTab:CreateToggle({
    Title   = "Anti Ragdoll",
    CurrentCurrentOption = false,
        ...
        pcall(function()
            ...
            local function findCounter2()
                for _, v in ipairs(getgc and getgc(true) or {}) do
                    if typeof(v) == "table" and rawget(v, "event") and rawget(v, "func") then return v
...
local HideNameToggle = CharTab:CreateToggle({
    Title   = "Hide Name",
    CurrentCurrentOption = false,
    Callback = function(v)
        pcall(function()
...
local AutoRespawnToggle = CharTab:CreateToggle({
    Title   = "Auto Respawn",
    CurrentCurrentOption = false,
    Callback = function(enabled)
        if not enabled then return
            ...
            task.spawn(function()
                while enabled do
                    ...
                    if hum and hum.Health <= 0 then
                        task.wait(6)
                        if enabled then sendAction("death_screen_request_respawn")
...
CharTab:CreateSection("PC HOLD (Z)")

local SnapToggle = CharTab:CreateToggle({
...
local SnapSlider = CharTab:CreateSlider({
    Title = "Snap:",
    Step  = 1,
    CurrentOption = { Min = 1, Max = 100, CurrentValue = 10 },
    Callback = function(v)
        snapDepth = v
...
local AutoFinishToggle = PlayerTab:CreateToggle({
    Title   = "Auto Finish",
    CurrentCurrentOption = false,
    Callback = function(v)
        autoFinishEnabled = v
        if v then
            applyToAll()
            Rayfield:Notify({ Title = "✅ Auto Finish Enabled", Content = "", Duration = 3 })

        else
            Rayfield:Notify({ Title = "❌ Auto Disabled", Content = "", Duration = 3 })
...
task.spawn(
    function()
        while task.wait(0.2) do
            pcall(function() BankBalance.Title = "Bank: " .. (ATMMoney() or 0) end)
            pcall(function() HandBalance.Title = "Hand: " .. (HandMoney() or 0) end)
...
pcall(function()
    local SkipCrateToggle = BuyTab:CreateToggle({
...
local function openCrateWithType(bulletType)
    ...
    if not crateOptions then
        ...
        Rayfield:Notify({Title = "❌ ไม่พบ Ammo Crate", Duration = 2})
    ...
    if not targetItem then
        ...
        Rayfield:Notify({Title = "❌ ไม่มี " .. bulletType .. " ใน crate", Duration = 2})
    ...
    if result then
        ...
        Rayfield:Notify({Title = "🔫 เปิด " .. bulletType .. " สำเร็จ", Duration = 2})

    else
        warn("❌ เปิด crate ล้มเหลว")
        Rayfield:Notify({Title = "❌ เปิด crate ไม่สำเร็จ", Duration = 2})
...
local bulletDropdown = BuyTab:CreateDropdown({
...
BuyTab:CreateButton({
    ...
    Callback = function()
        ...
        if useType == "Random" then
            ...
            Rayfield:Notify({Title = "สุ่มได้ " .. useType, Duration = 1})
...
MiscTab:CreateInput({
    ...
    Callback = function(v)
        if not v or v == "" then return

        local ids = {}
        for id in string.gmatch(v, "[%w%-]+") do table.insert(ids, id)
...
MiscTab:CreateButton({
    ...
    Callback = function()
        task.spawn(function()
            ...
            if ok then print("Claim All Quests Completed") else warn(err)
...
-- ── Config Management ─────────────────────────────────────────
MiscTab:CreateSection("Config Management")

local SaveBtn = MiscTab:CreateButton({
    Title    = "Save Config",
    Callback = function()
        if Config.Save then Config.Save(Config)

    end,
})
Config:Register("SaveConfig", SaveBtn)

local DeleteBtn = MiscTab:CreateButton({
    Title    = "Delete Config",
    Callback = function()
        if Config.Delete then Config.Delete(Config)
...
if Config.Load then Config.Load(Config)
